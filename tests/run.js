...
const panel = {
    x: 0, y: 200, width: 1920, height: 32, visible: true,
    get_pivot_point: () => [0, 0],
    ...
    ease(params) {
        ...
        const pending = {start: now, end: now + params.duration,
            from: this.y, to: params.y, onComplete: params.onComplete};
...
const Main = {
    ...
    layoutManager: {panelBox: panel, primaryMonitor: {y: 200}, hotCorners: [],
...
const Manager = new Function('Main', 'Config', 'Shell', 'Convenience', 'Clutter', 'GLib',
...
function fixture() {
    ...
    pointer = [50, 210];
    Object.assign(panel, {x: 0, y: 200, width: 1920, height: 32, visible: true});
...
let m = fixture();
m.hide(0, 'init');
assert(panel.y === 168 && !panel.visible, 'Hide uses nonzero monitor origin');
...
move(m, 50, 210);
advance(300);
assert(panel.visible && panel.y === 200 && animations === once + 1,
...
advance(200);
assert(panel.visible && panel.y === 200, 'Cancelled hide cannot run later');
...
advance(70);
const middleY = panel.y;
const obsoleteCompletion = transition.onComplete;
move(m, 50, 210);
assert(m._targetVisible && panel.y === middleY, 'Reentry reverses from the current position');
obsoleteCompletion();
assert(panel.visible && m._animationActive, 'Obsolete completion cannot hide reversed reveal');
advance(200);
assert(panel.y === 200 && watches.size === 1, 'Reversed reveal finishes fully visible');
...
m.destroy();
advance(500);
assert(panel.visible && panel.y === 200 && timers.size === 0 && watches.size === 0,
...
advance(500);
assert(panel.visible && panel.y === 200 && watches.size === 1,
