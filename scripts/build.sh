...
with zipfile.ZipFile('dist/hide-top-bar@whitehades.github.io.shell-extension.zip','w',zipfile.ZIP_DEFLATED) as z:
 files=[Path(p) for p in ('extension.js', 'panelVisibilityManager.js', 'intellihide.js', 'convenience.js', 'desktopIconsIntegration.js', 'prefs.js')]+[Path(p) for p in ('metadata.json','COPYING.txt','ATTRIBUTION.md','Settings.ui')]+list(Path('schemas').glob('*.gschema.xml'))+list(Path('locale').rglob('*.mo'))
